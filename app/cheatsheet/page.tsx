...
export const metadata: Metadata = {
  ...
  description: "9 Seiten, 14 Sektionen. Terminal-Befehle für macOS-Entwickler.",
  openGraph: {
    type: "website",
    url: "https://agent-bot.de/cheatsheet",
    title: "macOS Dev, Docker & AI — Ultimate Cheat Sheet",
    description: "Terminal-Cheat-Sheet für macOS-Entwickler. Kostenlos als PDF.",
    ...
    <div className="min-h-screen bg-white text-black font-sans">
      ...
      <div className="border-b border-black/10 bg-black/[0.02]">
        <div className="mx-auto max-w-6xl flex flex-wrap">
          {HIGHLIGHTS.map((s) => (<div key={s.label} className="flex-1 min-w-[120px] border-r last:border-r-0 border-black/10 px-6 py-5 text-center"><span className="block text-2xl font-black text-[#FF4F15]">{s.value}</span><span className="mt-1 block text-[10px] uppercase tracking-[0.2em] text-black/40 font-semibold">{s.label}</span></div>))}
        </div>
      </div>
      <section className="px-6 py-20 md:py-28">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {SECTIONS.map((section, i) => (<div key={section} className="flex items-start gap-3 border border-black/10 p-4 hover:border-[#FF4F15] transition-colors"><span className="text-[11px] font-bold text-[#FF4F15] mt-0.5 shrink-0">{String(i + 1).padStart(2, "0")}</span><span className="text-sm font-semibold">{section}</span></div>))}
          </div>
        </div>
      </section>
      <section className="bg-[#182332] text-white px-6 py-20 md:py-28">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {FEATURES.map((item) => (<div key={item.title} className="border border-white/10 p-6 hover:border-[#FF4F15] transition-colors"><h3 className="font-bold mb-2">{item.title}</h3><p className="text-sm text-white/50 leading-relaxed">{item.desc}</p></div>))}
          </div>
        </div>
      </section>
