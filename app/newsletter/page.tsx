...
const topics = [
  ...
  },
  {
    ...
    title: "News & Updates",
    description:
      "Die wichtigsten Releases und Ankündigungen der Woche – gefiltert und eingeordnet.",
  },
  {
    icon: "🧰",
  ...
  },
  {
    ...
    title: "Use Cases",
    description:
      "Echte Einsatzszenarien aus der Praxis – was funktioniert, was nicht und warum.",
  },
];
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-[#e8e8f0] font-sans">
      ...
      <div className="border-y border-[#1e1e2e] bg-[#0d0d18]">
        <div className="mx-auto flex max-w-3xl flex-wrap items-center justify-center divide-x divide-[#1e1e2e]">
          {[{num:"1.200+",label:"Abonnenten"},{num:"24",label:"Ausgaben"},{num:"5 min",label:"Lesezeit"},{num:"Freitags",label:"Erscheinungstag"}].map((s) => (
            <div key={s.label} className="px-8 py-6 text-center"><span className="block text-2xl font-black text-[#a78bfa]">{s.num}</span><span className="mt-1 block text-xs uppercase tracking-widest text-[#6b7280]">{s.label}</span></div>
          ))}
        </div>
      </div>
      <section className="px-6 py-20">
        <div className="mx-auto max-w-4xl">
          <div className="mb-12 text-center"><h2 className="text-3xl font-bold">Was dich erwartet</h2><p className="mt-2 text-[#9ca3af]">Jede Ausgabe bringt das Wesentliche auf den Punkt</p></div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {topics.map((t) => (<div key={t.title} className="rounded-xl border border-[#1e1e2e] bg-[#13131f] p-6 transition-all hover:-translate-y-1 hover:border-[#a78bfa]"><div className="mb-3 text-3xl">{t.icon}</div><h3 className="mb-2 font-semibold">{t.title}</h3><p className="text-sm leading-relaxed text-[#9ca3af]">{t.description}</p></div>))}
          </div>
        </div>
      </section>
      <section className="bg-[#0d0d18] px-6 py-20">
        <div className="mx-auto max-w-4xl">
          <div className="mb-12 text-center"><h2 className="text-3xl font-bold">Letzte Ausgaben</h2><p className="mt-2 text-[#9ca3af]">Ein Blick in die neuesten Deep Dives</p></div>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {issues.map((issue) => (<article key={issue.num} className="flex flex-col gap-3 rounded-xl border border-[#1e1e2e] bg-[#13131f] p-6 transition-all hover:-translate-y-1 hover:border-[#a78bfa]"><div className="flex items-center gap-2"><span className="text-xs font-bold uppercase tracking-widest text-[#a78bfa]">{issue.num}</span><time className="text-xs text-[#4b5563]" dateTime={issue.datetime}>{issue.date}</time></div><h3 className="font-semibold leading-snug">{issue.title}</h3><p className="flex-1 text-sm leading-relaxed text-[#9ca3af]">{issue.description}</p><div className="flex flex-wrap gap-1.5">{issue.tags.map((tag) => (<span key={tag} className="rounded bg-[rgba(167,139,250,0.1)] px-2 py-0.5 text-xs font-medium text-[#a78bfa]">{tag}</span>))}</div></article>))}
          </div>
        </div>
      </section>
