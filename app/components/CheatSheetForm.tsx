    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <input
          ...
          className="flex-1 min-w-0 border-2 border-black/20 bg-white px-4 py-3.5 text-sm text-black placeholder-black/30 outline-none transition-colors focus:border-[#FF4F15] disabled:opacity-50"
        ...
        <button
          ...
          className="bg-[#FF4F15] px-6 py-3.5 text-sm font-bold text-white uppercase tracking-[0.1em] transition-all hover:bg-[#182332] disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
        >
          {state === "loading" ? "Wird geladen…" : "Kostenlos holen"}
        </button>
      </div>
      {state === "error" && <p className="mt-2 text-xs text-red-600 font-medium">{errorMsg}</p>}
      <p className="mt-3 text-xs text-black/40">Kein Spam. Kein Tracking. Jederzeit abbestellbar.</p>
    </div>
  );
}
...
/* ── Data ───────────────────────────────────────────────── */
const SECTIONS = [
  "Navigation & Datei-Operationen", "Dateien lesen, suchen & bearbeiten",
  "macOS Spezifika & System", "Prozesse, Netzwerk & Ports",
  "Git — Basics", "Git — Fortgeschritten",
  "Homebrew & Package Management", "Node.js, pnpm & Vite",
  "Playwright", "Docker — Images & Container",
  "Docker Compose — Orchestrierung", "CI/CD — GitHub Actions",
  "Local AI & LLM (LM Studio)", "Power-User — Pipes, Aliases & .zshrc",
];
    ...
    <div className="min-h-screen bg-white text-black font-sans">
      <nav className="sticky top-0 z-50 w-full border-b border-black/10 bg-white/90 backdrop-blur-md">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-4">
          ...
          <a href="#download" className="bg-[#182332] text-white px-4 py-2 text-[11px] font-bold uppercase tracking-[0.15em] hover:bg-[#FF4F15] transition-colors">
            Zum Download
          </a>
        </div>
      </nav>
